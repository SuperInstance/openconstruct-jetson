...
OpenConstructJetson::OpenConstructJetson()
    : initialized_(false), running_(false), next_camera_id_(0),
      next_microphone_id_(0), cuda_stream_(nullptr) {}

OpenConstructJetson::~OpenConstructJetson() { cleanup_cuda(); }
...
void OpenConstructJetson::register_camera(int device_id, const char *name) {
  ...
  std::cout << "Registered camera '" << name << "' with device ID " << device_id
            << std::endl;
}
...
void OpenConstructJetson::register_microphone(int device_id, const char *name) {
  ...
  std::cout << "Registered microphone '" << name << "' with device ID "
            << device_id << std::endl;
}
...
void OpenConstructJetson::stop() { running_ = false; }
...
int parse_int_or(const std::string &s, int fallback) {
  try {
    ...
    std::cerr << "Warning: ignoring malformed numeric config value '" << s
              << "' (" << e.what() << "), using " << fallback << std::endl;
    return fallback;
  }
}
} // namespace
    ...
    if (pos != std::string::npos) {
      ...
      if (key == "model_path") {
        ...
        config_.audio_sample_rate =
            parse_int_or(value, config_.audio_sample_rate);
...
void OpenConstructJetson::init_cuda() {
  ...
  std::cout << "CUDA initialized on device " << config_.gpu_device_id
            << std::endl;
}
...
std::string OpenConstructJetson::format_gpu_status() {
  ...
  if (device_count > 0) {
    ...
    double utilization =
        (1.0 - static_cast<double>(free_mem) / total_mem) * 100.0;

    oss << "  Active device: " << config_.gpu_device_id << "\n";
    oss << "  Name: " << name << "\n";
    oss << "  Compute capability: " << major << "." << minor << "\n";
    oss << "  Memory: " << free_mem_gb << " GB free / " << total_mem_gb
        << " GB total (" << static_cast<int>(utilization) << "% used)\n";
  }
