 ...
 */
__global__ void
image_to_grayscale_kernel(const unsigned char *__restrict__ input,
  ...
  if (x < width && y < height) {
    ...
    output[out_idx] = static_cast<unsigned char>(0.299f * input[idx] +
                                                 0.587f * input[idx + 1] +
                                                 0.114f * input[idx + 2]);
  }
}
 ...
 */
__global__ void audio_preprocess_kernel(const float *__restrict__ input,
 ...
 */
__global__ void
feature_extraction_kernel(const unsigned char *__restrict__ grayscale,
                          float *__restrict__ features, int width, int height,
                          int feature_dim) {
...
void cuda_image_to_grayscale(const unsigned char *d_input,
  ...
  image_to_grayscale_kernel<<<grid, block, 0,
...
void cuda_audio_preprocess(const float *d_input, float *d_output, int length,
  ...
  audio_preprocess_kernel<<<blocks, threads, 0,
                            static_cast<cudaStream_t>(stream)>>>(
...
void cuda_extract_features(const unsigned char *d_grayscale, float *d_features,
  ...
  feature_extraction_kernel<<<blocks, threads, 0,
                              static_cast<cudaStream_t>(stream)>>>(
