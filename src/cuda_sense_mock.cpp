...
//
// This translation unit is compiled instead of cuda_sense.cu when the project is
// built without a CUDA toolkit (USE_CUDA=OFF), so the library still links and the
// unit tests can exercise the real C++ control flow on plain x86_64 hosts.
//
...
void cuda_get_device_properties(int device,
    ...
    if (compute_major) *compute_major = 7;
    if (compute_minor) *compute_minor = 2;
    if (total_mem) *total_mem = 8ULL * 1024 * 1024 * 1024; // 8 GB
}
