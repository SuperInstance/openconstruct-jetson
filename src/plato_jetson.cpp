...
private:
    ...
    std::string handle_camera(const std::vector<std::string>& args) {
        if (args.empty()) {
            return "ERROR: camera command requires arguments. Use 'camera list', 'camera add <id> <name>', or 'camera remove <id>'.";
        }
    ...
    std::string handle_microphone(const std::vector<std::string>& args) {
        if (args.empty()) {
            return "ERROR: microphone command requires arguments. Use 'microphone list', 'microphone add <id> <name>', or 'microphone remove <id>'.";
        }
    ...
    std::string handle_help() {
        ...
        oss << "  status              - Show system status (GPU, CPU, memory, thermal)\n";
